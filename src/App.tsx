              ...
              <PlannerView
                ...
                onGenerate={async () => {
                  ...
                  try {
                    ...
                    setTasksError(currentLanguage === 'pt' ? 'O planejador IA não está disponível agora. Tente novamente.' : 'AI Planner is unavailable right now. Please try again.')
                    setPlan(candidates)
                    setPlanPayments([])
                    setView('planner')
                  } finally {
...
function PlannerView({ plan, openTasks, aiLoading, isAuthenticated, onGenerate, onAddPlan }: { plan: Task[]; openTasks: Task[]; aiLoading: boolean; isAuthenticated: boolean; onGenerate: () => void; onAddPlan: () => void }) {
  ...
  return <div className="content-stack planner-page">
    ...
    <div className="planner-card">
      <div className="planner-hero"><div className="planner-bot"><Bot size={26} /></div><div><strong>{plan.length ? tr('plannerReady') : tr('openItems')}</strong><p>{plan.length ? (currentLanguage === 'pt' ? 'Ordem baseada nos prazos, prioridade e contexto das tarefas.' : 'Order based on due dates, priority, and task context.') : tr('generateOrder')}</p></div></div>
      {!plan.length ? <div className="planner-empty"><Sparkles size={22} /><div><strong>{tr('plannerEmptyTitle')}</strong><p>{tr('plannerEmptyDesc')}</p></div></div> : <div className="plan-list">{plan.map((task,index) => <div className="plan-item" key={task.id}><span className="plan-number">{index + 1}</span><div className="plan-date">{formatDate(task.dueDate)}</div><div className="plan-main"><strong>{task.title}</strong><small>{task.client}</small></div><span className={task.priority === 'high' ? 'plan-priority high' : task.priority === 'medium' ? 'plan-priority medium' : 'plan-priority low'}>{task.priority}</span></div>)}</div>}
      {!isAuthenticated && plan.length > 0 && <div className="planner-note"><Bot size={14} /> {tr('plannerFallback')}</div>}
      <div className="planner-actions">{plan.length > 0 && <button className="primary-button" onClick={onAddPlan}><CheckCircle2 size={17} /> {tr('addPlanToLifeDue')}</button>}</div>
    </div>
  </div>
}
