  ...
  const createPlan = async () => {
    ...
    try {
      ...
      if (data?.aiUsage) {
        ...
        setAiLimitNotice(nextAiUsage.remaining === 0)
      }
      ...
      {view === 'home' ? (
        ...
        <div className="workspace">
          ...
          <main className="main-content">
            ...
            {aiLimitNotice && (
              <div className="error-banner" role="status" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, margin: '0 0 16px' }}>
                <span>{tr('aiLimitReachedNotice')}</span>
                <button className="text-button" type="button" onClick={() => setAiLimitNotice(false)}>{tr('dismiss')}</button>
              </div>
            )}
