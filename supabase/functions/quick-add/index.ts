...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const prompt = [
      'You are LifeDue task and payment extraction engine.',
      'Today is ' + today + '.',
      'User timezone: ' + timezone + '.',
      'Convert the user note into actionable items.',
      'Return one item per distinct task or payment.',
      'Use kind=payment only for explicit money/payment/charging/receiving/invoice mentions.',
      'Never invent an amount, client, or deadline.',
      'Keep client names exactly as written by the user.',
      'Never translate, anglicize, normalize, autocorrect, or replace a person\'s name.',
      'Preserve accents and spelling exactly.',
      'Resolve relative dates from the supplied today.',
      'If only a weekday is given, use its next occurrence.',
      'Payment amount/currency should be null when not stated.',
      'Use concise action titles without client names.',
      'User note:',
      text,
    ].join('\\n')
